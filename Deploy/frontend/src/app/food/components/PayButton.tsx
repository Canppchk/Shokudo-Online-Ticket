...
function PaymentButton({name}:UserProps) {
  ...
  const handlePayment = async () => {
    ...
    try {
      const response = await fetch('http://163.221.29.107:80/create-qr', {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    const checkPaymentStatus = async () => {
      ...
      try {
        const response = await fetch(`http://163.221.29.107:80/order-status/${merchantPaymentId}`);
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        ...
        if (data.data.status === "COMPLETED" && !ticketCreated.current) {
          ...
          const ticketResponse = await fetch(`http://163.221.29.107:80/ticket/${name}`, {
            method: "POST",
            headers: {
              'Content-Type': 'application/json',
            },
